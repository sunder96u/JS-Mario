...
Game.Enemy.prototype.CollideCheck = function() {
    ...
    let xMarioD = Game.Main.X - this.X, yMarioD = Game.Main.Y - this.Y;
    if (xMarioD > -this.Width * 2 - 4 && xMarioD < this.Width * 2 + 4) {
        if (yMarioD > -this.Height && yMarioD < Game.Main.Height) {
            if (this.Type !== Game.Enemy.Spiky && Game.Main.Ya > 0 && yMarioD <= 0 && (!Game.Main.OnGround || !Game.Main.WasOnGround)) {
...
Game.Shell.prototype.CollideCheck = function() {
	...
	if (xMarioD > -16 && xMarioD < 16) {
        if (yMarioD > -this.Height && yMarioD < Game.Main.Height) {
			if (Game.Main.Ya > 0 && yMarioD <= 0 && (!Game.Main.OnGround || !Game.Main.WasOnGround)) {
				Game.Main.Stomp(this);
				if (this.Facing !== 0) {
					this.Xa = 0;
					this.Facing = 0;
...
Game.Shell.prototype.Release = function(mario) {
    this.Carried = false;
    this.Facing = Game.Main.Facing;
